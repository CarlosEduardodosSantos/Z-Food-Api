...
namespace APIAlturas.Controllers
{
    ...
    [Route("api/v2/Pedido")]
    public class PedidoV2Controller : Controller
    {
        private readonly PedidoDAO _pedidosDao;
        //private readonly OneSignalService _oneSignalService;

        public PedidoV2Controller([FromServices]PedidoDAO pedidosDao)
        {
        ...
        public object Get(string id, Guid token)
        {
            ...
            try
            {
                return new
                {
                    ...
                    totalVoucher = data.DescontoTotal,
                    type = !string.IsNullOrEmpty(data.OrderType) ? data.OrderType : data.IsEntrega ? "DELIVERY" : "  RETIRA",
                    customer = new
                    {
                        id = data.User.UserID,
